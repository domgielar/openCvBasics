import os
import cv2
import numpy as np


img = cv2.imread(os.path.join('.','data','jaysontatum.png'))
#shoes the edges of the pitcures greater ratio between the number cuases less edges to be shown and viceversa
img_edge = cv2.Canny(img,200,500 )

#creates bigger lines for edges you can edit the thickness by editin the np.ones values
img_edge_d=cv2.dilate(img_edge,np.ones((3,3), dtype=np.int8))

#creates smaller lines for edges
img_edge_e = cv2.erode(img_edge_d,np.ones((3,3), dtype=np.int8))

cv2.imshow('img',img)
cv2.imshow('img_edge',img_edge)
cv2.imshow('img_edge_d',img_edge_d)
cv2.imshow('img_edge_e',img_edge_e)


cv2.waitKey(0)